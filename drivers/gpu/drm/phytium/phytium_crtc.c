...
int phytium_crtc_init(struct drm_device *dev, int phys_pipe)
{
	...
	if (IS_X100(priv)) {
		phytium_crtc->dc_hw_config_pix_clock = x100_dc_hw_config_pix_clock;
		phytium_crtc->dc_hw_disable = x100_dc_hw_disable;
		phytium_crtc->dc_hw_reset = NULL;
		priv->dc_reg_base[phys_pipe] = X100_DC_BASE(phys_pipe);
		priv->dcreq_reg_base[phys_pipe] = X100_DCREQ_BASE(phys_pipe);
		priv->address_transform_base = X100_ADDRESS_TRANSFORM_BASE;
	} else if (IS_E2000(priv)) {
		phytium_crtc->dc_hw_config_pix_clock = e2000_dc_hw_config_pix_clock;
		phytium_crtc->dc_hw_disable = e2000_dc_hw_disable;
		phytium_crtc->dc_hw_reset = e2000_dc_hw_reset;
		priv->dc_reg_base[phys_pipe] = E2000_DC_BASE(phys_pipe);
		priv->dcreq_reg_base[phys_pipe] = 0x0;
		priv->address_transform_base = E2000_ADDRESS_TRANSFORM_BASE;
	}
