 ...
 */
extern u32 gui32Alloc;
/**
 * @brief The number of calls to free (used if IMG_MALLOC_CHECK)
 */
extern u32 gui32Free;

static void *img_malloc_check(size_t size)
{
...
static void *img_calloc_check(size_t nelem, size_t elem_size)
{
 ...
 *
 * This behaviour can be used to test allocation process
 * (e.g. set gui32AllocFails to 5 and the 5th allocation after that will fail)
 *
 * @warning your code needs to define this variable (e.g. u32 guiAllocFails = 0;).
 * If your code is C++ don't forget the extern "C"
 */
extern u32 gui32AllocFails;
...
static void *img_malloc_test(size_t size)
{
	if (img_test_alloc()) {
#ifdef IMG_MALLOC_CHECK
...
static void *img_calloc_test(size_t nelem, size_t elem_size)
{
	if (img_test_alloc()) {
#ifdef IMG_MALLOC_CHECK
...
/*
 * @note maybe this should be done in a function to
 * know if it worked with another way than just assert...
 */
#define IMG_UINT64_TO_UINT32(ui64Check)\
	(IMG_ASSERT(((ui64Check) >> 32) == 0), (u32)(ui64Check))
